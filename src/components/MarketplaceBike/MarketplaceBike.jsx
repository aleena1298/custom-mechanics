...
export default function MarketplaceBike({
  ...
  useLayoutEffect(() => {
    ...
    const hoverIn = (element, options) => {
      gsap.to(element, {
        ...
        boxShadow:
          options.shadow || "0px 0px 15px rgba(255, 40, 40, 0.6)",
        duration: 0.3,
        ease: "power2.out",
      });
    };
    ...
    return () => {
      ...
      secondary?.removeEventListener("mouseenter", secondaryEnter);
      secondary?.removeEventListener("mouseleave", secondaryLeave);
  ...
  if (variant === "parallax") {
  return (
    <div className="bike-parallax-main" ref={sectionRef}>
      <div
      ...
      <div className="bike-parallax-text text">
        <div className="bike-parallax-custom">
          <h1 ref={(el) => setHeadingRef(el, 0)}>
            <span>{head1}</span>
          </h1>
        </div>

        <div className="bike-parallax-mechanics">
          <h1 ref={(el) => setHeadingRef(el, 1)}>
            <span>{head2}</span>
          </h1>
        </div>
...
/* STANDARD MUST ALSO BE RETURNED */
return (
  <div className="standard-bike-main" ref={sectionRef}>
